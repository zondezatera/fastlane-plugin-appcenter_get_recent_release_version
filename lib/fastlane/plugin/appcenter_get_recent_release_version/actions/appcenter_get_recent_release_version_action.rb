...
require 'net/http'
require 'json'
require_relative '../helper/appcenter_get_recent_release_version_helper'

module Fastlane
  module Actions
    class AppcenterGetRecentReleaseVersionAction < Action
      def self.run(params)
        api_token = params[:api_token]
        owner_name = params[:owner_name]
        app_name = params[:app_name]

        url = "https://api.appcenter.ms/v0.1/apps/#{owner_name}/#{app_name}/recent_releases"
        uri = URI(url)
        ...
        when Net::HTTPSuccess
          ...
          data_response  = json_response.first
          data_response['version'].to_i
        else
          response.value
        end
      end
